...
// ---------------------------------------------------------
const DB_NAME = 'POSDatabase_v8'; // Added salesTargets collection
const DB_VERSION = 1; // Reset version for new DB
...
class LocalDB {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('modifiers')) {
                    const store = db.createObjectStore('modifiers', { keyPath: 'id' });
                    store.createIndex('storeId', 'storeId', { unique: false });
                    store.createIndex('syncStatus', 'syncStatus', { unique: false });
                }
                ...
                if (!db.objectStoreNames.contains('salaries')) {
                    const store = db.createObjectStore('salaries', { keyPath: 'id' });
                    store.createIndex('date', 'date', { unique: false });
                    store.createIndex('storeId', 'storeId', { unique: false });
                    store.createIndex('cashier', 'cashier', { unique: false });
                    store.createIndex('syncStatus', 'syncStatus', { unique: false });
                ...
                // Sales Targets (NEW) — per product per month goals set by admin
                if (!db.objectStoreNames.contains('salesTargets')) {
                    const store = db.createObjectStore('salesTargets', { keyPath: 'id' });
                    store.createIndex('productId', 'productId', { unique: false });
                    store.createIndex('storeId', 'storeId', { unique: false });
    ...
    async getPendingSync(storeName) {
        return new Promise((resolve, reject) => {
            ...
            // Check if store exists in current DB connection
            if (!this.db.objectStoreNames.contains(storeName)) {
                // If checking sync for a store that doesn't exist, just return empty
                // This happens during initial migration or version mismatches
                console.warn(`LocalDB: getPendingSync called for non-existent store '${storeName}'`);
                resolve([]);
                return;
            }
            ...
            try {
                ...
                console.error(`LocalDB: Error accessing '${storeName}' for sync:`, error);
                resolve([]);
            }
        });
    }
...
class OfflineDB {
    ...
    async refreshCollectionFromCloud(collectionName, storeId) {
        try {
            ...
            const snap = await getDocs(q);
            console.log(`Cloud Query Result (${collectionName}): ${snap.size} docs found`);
    ...
    injectStatusIndicator() {
        // Skip for cashier role and login page (to keep UI clean as requested)
        if (window.location.pathname.includes('cashier.html') || document.querySelector('.login-wrapper')) return;
    ...
    /**
     * Hard Reset Store - Deletes ALL data for the current store
     * Uses parallel individual deletes to avoid batch failures due to permissions.
     */
    async hardResetStore() {
        if (!this.isOnline) {
            throw new Error("Must be online to perform a hard reset");
        }

        const storeId = this.getCurrentStoreId();
        const currentUser = auth.getCurrentUser();

        if (!storeId || storeId === 'default_store' || !currentUser) {
            throw new Error("Invalid session state");
        }

        console.warn(`⚠️ STARTING HARD RESET FOR STORE: ${storeId}`);
        ...
        try {
            ...
            const qUsers = query(usersRef, where('storeId', '==', storeId));
            const usersSnap = await getDocs(qUsers);

            if (!usersSnap.empty) {
                const deletePromises = usersSnap.docs.map(async (d) => {
                    // Skip current admin
                    if (d.data().username === currentUser.username) return;

                    try {
                        await deleteDoc(d.ref);
                        successCount++;
                    } catch (e) {
                        console.warn(`Failed to delete user ${d.id}:`, e);
                        failureCount++;
                    }
                });
            ...
            // 2. Delete all other collections completely for this store
            for (const colName of collections) {
                ...
                if (!snap.empty) {
                    ...
                    // Delete in parallel chunks of 10 to avoid overwhelming connection but maintain speed
                    ...
                    for (let i = 0; i < docs.length; i += 10) {
                        ...
                        await Promise.all(chunk.map(async (d) => {
                            try {
                                ...
                                console.warn(`Failed to delete ${colName}/${d.id}:`, e);
                                failureCount++;
                            }
                        }));
                    }
            ...
            // 3. Clear Local IndexedDB entirely for these object stores
            for (const colName of [...collections, 'users']) {
                const localItems = await this.local.getAll(colName);
                for (const item of localItems) {
                    if (item.storeId === storeId) {
                        // Skip current admin for users
                        if (colName === 'users' && item.username === currentUser.username) continue;
                        await this.local.delete(colName, item.id);
                    }
                }
            }

            if (failureCount > 0) {
                console.warn(`Hard reset finished with ${failureCount} errors.`);
                // We don't throw here, we let it finish so the user gets a "mostly done" state
            }
