...
let pullToRefresh = {
    ...
    getScrollContainer() {
        ...
        // Check which container is actually scrolling
        if (adminMain && getComputedStyle(adminMain).overflowY !== 'visible') return adminMain;
        if (cashierMain && getComputedStyle(cashierMain).overflowY !== 'visible') return cashierMain;

        // Fallback to body/doc element (native scroll)
    ...
    onTouchStart(e) {
        ...
        // CRITICAL: Ignore touches on mobile menu toggle button
        const mobileMenuToggle = document.getElementById('mobileMenuToggle');
        if (mobileMenuToggle && (mobileMenuToggle === e.target || mobileMenuToggle.contains(e.target))) {
            return; // Don't intercept menu button touches
        }
    ...
    async triggerRefresh() {
        ...
        this.indicator.querySelector('.refresh-text').textContent = 'Clearing cache...';

        try {
            // HARD CACHE CLEAR FOR iOS
            showLoading('Clearing cache and refreshing...');
            ...
            // 2. Unregister service workers (they'll re-register on reload)
            ...
            this.indicator.querySelector('.refresh-text').textContent = 'Syncing data...';

            // 3. Force cloud sync for all collections
            if (typeof db !== 'undefined' && db.syncPendingData) {
                await db.syncPendingData();
            }

            // 4. Reload current view/tab
            ...
            hideLoading();
            showToast('✅ Cache cleared & data refreshed!', 'success');
