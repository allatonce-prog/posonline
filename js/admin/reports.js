...
async function loadReports() {
    ...
    // Calculate sales (regular sales + collectible payments received)
    ...
    // Note: Collectibles debt is NOT subtracted from profit
    // Sales already includes collectible_payment transactions (money actually received)
    const weekNetProfit = weekSales - weekExpensesTotal;
    const monthNetProfit = monthSales - monthExpensesTotal;
            ...
            return `
                <tr>
                    <td colspan="5" style="padding: 0; border: none;">
                        <div class="top-product-card" style="border: 1px solid var(--gray-200); border-radius: var(--radius-md); padding: 1rem; margin-bottom: 0.75rem; background: var(--white); display: flex; flex-direction: column; gap: 0.75rem; position: relative;">
                            
                            <!-- Header: Rank & Revenue -->
                            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                <div style="display: flex; align-items: center; gap: 0.75rem;">
                                    <div class="rank-badge" style="background: var(--primary); color: white; width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 0.9rem;">${index + 1}</div>
                                    <div style="font-weight: 700; color: var(--dark); font-size: 1.1rem;">
                                        ${formatCurrency(product.revenue)}
                                    </div>
                                </div>
                                <span class="category-tag-modern" style="font-size: 0.75rem; background: var(--gray-100); color: var(--gray-700); padding: 2px 8px; border-radius: 12px;">${escapeHtml(product.category)}</span>
                            </div>

                            <!-- Product Info -->
                            <div style="display: flex; gap: 1rem; align-items: center;">
                                <div class="product-img-mini" style="width: 50px; height: 50px; background: var(--gray-50); border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; overflow: hidden; flex-shrink: 0;">
                                    ${productImage ? `<img src="${productImage}" alt="" style="width: 100%; height: 100%; object-fit: cover;">` : '📦'}
                                </div>
                                <div style="flex: 1;">
                                    <div class="product-name-modern" style="font-weight: 600; color: var(--dark); margin-bottom: 2px; line-height: 1.2;">${escapeHtml(product.name)}</div>
                                    <div class="product-sku-modern" style="font-size: 0.8rem; color: var(--gray-500);">SKU: ${productMap[product.productId]?.sku || 'N/A'}</div>
                                </div>
                            </div>

                            <!-- Progress Bar -->
                            <div class="sales-progress-container" style="margin-top: 0.25rem;">
                                <div style="display: flex; justify-content: space-between; font-size: 0.8rem; margin-bottom: 4px;">
                                    <span style="color: var(--gray-600);">Sold</span>
                                    <span class="sales-count-modern" style="font-weight: 600; color: var(--primary);">${product.quantity} units</span>
                                </div>
                                <div class="progress-bar-modern" style="height: 6px; background: var(--gray-100); border-radius: 3px; overflow: hidden;">
                                    <div class="progress-fill-modern" style="width: ${percentage}%; height: 100%; background: var(--primary); border-radius: 3px;"></div>
                                </div>
                            </div>
                        </div>
                    </td>
